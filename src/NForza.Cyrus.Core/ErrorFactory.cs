...
public static class ErrorFactory<T>
{
    ...
    public static Error TenantMismatch<TEntity>(int errorId = 0)
    {
        var entityName = typeof(TEntity).Name;
        return Error.Factory.CreateFor<T>($"The requested '{entityName}' does not belong to the current tenant.", ErrorType.Forbidden, errorId);
    }

    public static Error TenantMismatch(object entity, int errorId = 0)
    {
    ...
    public static Error DeleteConflict<TReference>(object entity, int errorId = 0)
    {
        var entityName = entity.GetType().Name;
        var referenceName = typeof(TReference).Name;
        var selfReferenceMessage = "";

        if (entityName == referenceName)
        {
            selfReferenceMessage = " (Self-reference)";
        }

        return Error.Factory.CreateFor<T>($"The '{entityName}' is in use by '{referenceName}' and cannot be deleted.{selfReferenceMessage}", ErrorType.Conflict, errorId);
    }
